    ...
    <body class="min-h-screen bg-white dark:bg-zinc-800">
        <flux:header container class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <flux:navbar class="-mb-px max-lg:hidden">
                ...
                <flux:navbar.item icon="clock" :href="route('trade-history')" :current="request()->routeIs('trade-history')" wire:navigate>
                    {{ __('Trade History') }}
                </flux:navbar.item>
                ...
                @if(auth()->user()->isAdmin())
                    <flux:navbar.item icon="banknotes" :href="route('admin.earnings')" :current="request()->routeIs('admin.earnings')" wire:navigate>
                        {{ __('Earnings Overview') }}
                    </flux:navbar.item>
                    <flux:navbar.item icon="users" :href="route('admin.users')" :current="request()->routeIs('admin.users')" wire:navigate>
                        {{ __('User Management') }}
                    </flux:navbar.item>
                    <flux:navbar.item icon="cog" :href="route('admin.settings')" :current="request()->routeIs('admin.settings')" wire:navigate>
        ...
        <flux:sidebar collapsible="mobile" sticky class="lg:hidden border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            <flux:sidebar.nav>
                <flux:sidebar.group :heading="__('Platform')">
                    ...
                    <flux:sidebar.item icon="clock" :href="route('trade-history')" :current="request()->routeIs('trade-history')" wire:navigate>
                        {{ __('Trade History') }}
                    </flux:sidebar.item>
                ...
                @if(auth()->user()->isAdmin())
                    <flux:sidebar.group :heading="__('Admin')">
                        <flux:sidebar.item icon="banknotes" :href="route('admin.earnings')" :current="request()->routeIs('admin.earnings')" wire:navigate>
                            {{ __('Earnings Overview') }}
                        </flux:sidebar.item>
